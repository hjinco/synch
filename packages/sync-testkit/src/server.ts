...
export interface TestServer { processId: number; baseUrl: string; logs(): string; close(): Promise<void>; }
...
export async function startServer(runtime: Runtime, checkoutRoot = defaultRoot): Promise<TestServer> {
  const apiRoot = path.join(checkoutRoot, "apps/api");
  ...
  function launch(command: string, args: string[], extra: Record<string, string> = {}) {
    const launched = spawn(command, args, { cwd: directory, env: { ...env, ...extra }, stdio: ["ignore", "pipe", "pipe"], detached: process.platform !== "win32" });
    launched.stdout?.on("data", chunk => { output = (output + chunk).slice(-64 * 1024); });
    launched.stderr?.on("data", chunk => { output = (output + chunk).slice(-64 * 1024); });
    return launched;
  }
  async function close() {
    if (child?.pid && child.exitCode === null && child.signalCode === null) {
      ...
      const timer = setTimeout(() => child && signalTree(child, "SIGKILL"), 5_000);
  ...
  try {
    ...
    if (runtime === "node") {
      ...
      const migrationTimer = setTimeout(() => child && signalTree(child, "SIGTERM"), 30_000);
    ...
    while (Date.now() < deadline) {
      ...
      try {
        ...
        if (response.ok) return { processId: child.pid!, baseUrl, logs: () => output, close };
