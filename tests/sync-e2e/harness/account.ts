...
export async function createVault(baseUrl: string, cookie: string) {
  ...
  return {
    id: vault.id, key: wrapped.remoteVaultKey,
    async unlock() {
      const response = await fetch(`${baseUrl}/v1/vaults/${vault.id}/bootstrap`, { headers });
    ...
    async token(localVaultId: string): Promise<SyncTokenResponse> {
      const response = await fetch(`${baseUrl}/v1/sync/token`, {
        method: "POST", headers, body: JSON.stringify({ vaultId: vault.id, localVaultId }),
      });
